...
**Από 2026-07-30 ο Worker γράφει ΜΟΝΟΣ ΤΟΥ την επαφή + την εισερχόμενη επικοινωνία τη στιγμή της
υποβολής** (worker/lib/crm-lead.mjs — το email γράφει «Επαφή #id»). Το 7β άρα: βρίσκει την επαφή
με dedupe (τη ΞΑΝΑχρησιμοποιεί, δεν διπλογράφει), δημιουργεί ΜΟΝΟ τη ζήτηση, και ΔΕΝ γράφει
δεύτερη επικοινωνία (το worklist entry δεν έχει `comm`· το crm-post.mjs το προσπερνά). Η δική του
δημιουργία επαφής είναι πλέον fallback για την περίπτωση που το CRM write του Worker απέτυχε
(το email τότε γράφει «ΔΕΝ καταχωρήθηκε» με κόκκινο).
