...
/* ΧΩΡΙΣ ΦΩΤΟΓΡΑΦΙΕΣ, ΕΚΤΟΣ SITE.

   Ένα ακίνητο μπαίνει στο CRM τη μέρα που υπογράφεται και οι φωτογραφίες
   έρχονται μετά τη λήψη. Στο ενδιάμεσο έβγαινε κανονικά στο site, και
   μάλιστα ΠΡΩΤΟ: είναι το πιο πρόσφατο, οπότε έπιανε τη θέση 0 στα
   «Νεότερα» και στις «Νέες καταχωρίσεις» της αρχικής. Μια κάρτα χωρίς
   φωτογραφία δεν πουλάει το ακίνητο, δείχνει ένα γραφείο που δεν πρόλαβε.

   Το φίλτρο μπαίνει στο ΣΕΡΒΙΡΙΣΜΑ, όχι στο χτίσιμο: στο KV το feed μένει
   ολόκληρο, γιατί το χρειάζονται εσωτερικοί καταναλωτές που δεν δείχνουν
   τίποτα δημόσια (η εκτίμηση διαβάζει comparables από το KV, η φόρμα της
   εκτίμησης προσυμπληρώνει το ίδιο το ακίνητο από το feed στο forms host).
   Δημόσια το κόβουν: serveFeed στο worker/index.mjs, το sitemap και η
   σελίδα ακινήτου στο lib/seo.mjs, οι προτάσεις του lib/lead-reply.mjs.
   Μόλις ανέβει η πρώτη φωτογραφία στο CRM, το ακίνητο εμφανίζεται μόνο
   του στο επόμενο webhook/cron. */
export function hasPhotos(l) {
	return Array.isArray(l?.images) && l.images.length > 0;
}

export function publicListings(listings) {
	return (listings || []).filter(hasPhotos);
}
