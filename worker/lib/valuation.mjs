...
/* ΜΕΖΟΝΕΤΑ Ή ΒΙΛΑ ΣΕ ΟΡΙΖΟΝΤΙΑ ΙΔΙΟΚΤΗΣΙΑ — ΑΛΛΗ ΜΕΘΟΔΟΣ.
   Το κατώφλι της αξίας γης προϋποθέτει ότι ο ιδιοκτήτης ΕΧΕΙ τη γη. Σε
   οριζόντια ιδιοκτησία σε συγκρότημα δεν την έχει: πουλά ποσοστό
   εξ αδιαιρέτου που δεν ελέγχει, και το υπόλοιπο δόμησης δεν είναι δικό
   του. Με τη μέθοδο της μονοκατοικίας το νούμερο ανεβαίνει σε τιμή που
   δεν πιάνεται στην αγορά. Η φόρμα κρύβει τα ίδια πεδία με το ίδιο
   κριτήριο (applyOwnership στο forms/ektimisi.html) — αν αλλάξει το ένα,
   αλλάζει και το άλλο. */
const HORIZONTAL_OWNERSHIP = "Οριζόντια ιδιοκτησία σε συγκρότημα";

const METHOD_HORIZONTAL = `ΜΕΘΟΔΟΣ — ΚΑΘΑΡΑ ΣΥΓΚΡΙΤΙΚΑ, ΧΩΡΙΣ ΚΑΤΩΦΛΙ ΑΞΙΑΣ ΓΗΣ. Το ακίνητο είναι ΟΡΙΖΟΝΤΙΑ ΙΔΙΟΚΤΗΣΙΑ ΣΕ ΣΥΓΚΡΟΤΗΜΑ: δεν έχει δικό του οικόπεδο ούτε δικό του υπόλοιπο δόμησης.
- Εκτίμησε με συγκριτικά κατοικιών €/τ.μ. κτίσματος, όπως διαμέρισμα που τυχαίνει να έχει περισσότερα επίπεδα.
- ΜΗΝ υπολογίσεις αξία γης (οικόπεδο × ΣΔ) και ΜΗΝ τη βάλεις ως κατώφλι: ο κανόνας «μονοκατοικία ή μεζονέτα δεν αξίζει λιγότερο από το οικόπεδό της» ΔΕΝ ισχύει εδώ. Τα στοιχεία του οικοπέδου λείπουν επίτηδες — ΜΗΝ τα ζητήσεις στα missing_info και μην τα σχολιάσεις.
- ΒΑΡΑΙΝΟΥΝ ΑΝΤ' ΑΥΤΩΝ: επίπεδα και εσωτερική σκάλα (η σκάλα είναι και μειονέκτημα για μεγαλύτερους αγοραστές), ανεξάρτητη είσοδος, κήπος ή αυλή αποκλειστικής χρήσης, ταράτσα, θέση στάθμευσης, ποιότητα και συντήρηση του συγκροτήματος, κατάσταση και έτος, ενεργειακή κλάση, θέα.
- Η ΙΔΙΑ μεζονέτα σε αυτοτελές οικόπεδο θα άξιζε περισσότερο. Αν ο ιδιοκτήτης συγκρίνει με τέτοιες, πες το στο advice.
ΔΕΝ ΥΠΑΡΧΟΥΝ ΕΔΩ: όροφος, ασανσέρ, στοιχεία οικοπέδου. Μην τα ζητήσεις και μην τα σχολιάσεις.`;

function methodBrief(prop) {
	const ownLand = prop.profile === "house" || prop.profile === "villa";
	if (ownLand && prop.ownershipForm === HORIZONTAL_OWNERSHIP) return METHOD_HORIZONTAL;
	return METHOD_BRIEF[prop.profile] || METHOD_BRIEF.apartment;
}
